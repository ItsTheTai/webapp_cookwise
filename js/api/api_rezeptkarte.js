...
/*
This section sets the onClick-function for the
'add to shopping list'-Button.

When pressed, the button gets the ingredients,
title and selected portion amount of the current recipe
and saves it to localStorage in a JSON-Object.

LocalStorage is then used in shoppinglist.js
to render the recipes into the shoppinglist
*/
...
function handleShoppingListButton() {
    ...
    showVisualFeedback(
        shoppinglistBtn,
        "success",
        `"${recipeTitle}" zur Einkaufsliste hinzugefügt!`
    );
}
...
function renderRecipeDetail(recipe) {
    ...
    try {
        ...
        const currentServings = portionsCountEl
            ? parseInt(portionsCountEl.textContent)
            : 1;

        if (mainContainer && recipe.zutaten) {
            // Build the core unordered list structure from scratch dynamically
            mainContainer.innerHTML =
                '<ul class="ingredients-list list-unstyled m-0 p-0"></ul>';
            ...
            recipe.zutaten.forEach((item, index) => {
                ...
                const initialDisplayedAmount = amountOnOneServing
                    ? Number((amountOnOneServing * currentServings).toFixed(2))
                    : '';
                ...
                li.innerHTML = `
                    ...
                    <span>
                        <strong class="ingredient-amount" data-base="${amountOnOneServing}">
                            ${initialDisplayedAmount} ${unit}
                        </strong> ${name}
                    </span>
                `;
                ...
                // Builds a JSON-Object for every ingredient and saves it
                // into the ingredient variable that is then stored in localStorage
                let currentIngredient = {
                    "name": name,
                    "unit": unit,
                    "baseAmount": amountOnOneServing,
                    "recipe": recipe.titel
                };
            ...
            console.log(
                "Prep tab rendered and synced with portions engine successfully."
            );
        }
    } catch (e) {
    ...
    try {
        ...
        if (stepsContainer && recipe.schritte) {
            ...
            if (allSteps.length > 0) {
                ...
                stepsContainer.innerHTML =
                    "<li>Keine Schritte verfügbar.</li>";
            }
        }
    } catch (e) {
    ...
    try {
        ...
        console.error("Error in Serve tab block:", e);
    }
        ...
        favoriteBtn.addEventListener("click", (e) => {
            ...
            if (!isAlreadyFavorite) {
                ...
                showVisualFeedback(
                    favoriteBtn,
                    "success",
                    `"${recipe.titel}" zu Favoriten hinzugefügt!`
                );
            } else {
                showVisualFeedback(
                    favoriteBtn,
    ...
    if (plannerBtn) {
        ...
        // Check if recipe is already in the AI Planner
        let plannerRecipes =
            JSON.parse(localStorage.getItem("ai_planner_recipes")) || [];
        ...
        plannerBtn.addEventListener("click", (e) => {
            ...
            let currentPlannerRecipes =
                JSON.parse(localStorage.getItem("ai_planner_recipes")) || [];

            const isAlreadyAdded =
                currentPlannerRecipes.some(item => item.id === recipe.id);

            if (!isAlreadyAdded) {

                // Den vollständigen Rezept-Detail-Datensatz speichern.
                // Dadurch bleiben u. a. Portionen, Zutaten mit Mengen/Einheiten
                // sowie Vorbereitungs- und Zubereitungsschritte mit Zeiten erhalten.
                currentPlannerRecipes.push(recipe);

                localStorage.setItem(
                    "ai_planner_recipes",
                    JSON.stringify(currentPlannerRecipes)
                );
                ...
                showVisualFeedback(
                    plannerBtn,
                    "success",
                    `"${recipe.titel}" zum Kochplaner hinzugefügt!`
                );
            } else {
                showVisualFeedback(
                    plannerBtn,
                    "warning",
                    "Bereits im Kochplaner vorhanden!"
                );
            }
        });
    }
...
Generates floating visual notification banners
without blocking UI flow
*/
...
function showVisualFeedback(element, status, message) {
    ...
    // 2. Look for or create the global toast layout
    ...
    const themeClass =
        status === "success"
            ...
            : "alert-warning text-warning-emphasis";
    ...
    alertBox.className =
        `alert ${themeClass} alert-dismissible fade show shadow d-flex align-items-center mb-2`;

    alertBox.role = "alert";

    alertBox.style.cssText =
        "border-radius: 0.6rem; background-color: rgba(255,255,255,0.95); backdrop-filter: blur(4px);";
    ...
    alertBox.innerHTML = `
        ...
        <button
            ...
            aria-label="Close"
            style="transform: scale(0.8); margin-top: -2px;">
        </button>
    `;
    ...
    // 4. Clean notification component after 3 seconds
    setTimeout(() => {
